#!/usr/bin/env python3
"""
md_to_html.py — Convert Markdown to polished, self-contained HTML.

Themes:
  - docs          : Clean technical documentation with sidebar TOC
  - presentation  : 16:9 slide deck with keyboard navigation (auto-splits on H2)

Part of md2html — https://github.com/YoungL524/md2html
"""
...
VALID_THEMES = ("docs", "presentation")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Convert Markdown to polished HTML with built-in themes.",
    )
    ...
    p.add_argument(
        ...
        default="docs",
        help="Theme to use (default: docs).",
    )
    p.add_argument(
        "--inline-images",
        action="store_true",
    ...
    p.add_argument(
        "--toc",
        action="store_true",
        help="Render a table of contents (auto-on for theme=docs).",
    )
    p.add_argument(
        "--no-toc",
        action="store_true",
    ...
    p.add_argument(
        "--katex",
        action="store_true",
        help="Render math via KaTeX (loaded from CDN).",
    )
    p.add_argument(
        "--highlight-style",
        default="pygments",
